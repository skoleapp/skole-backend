...
def test_mark_all_as_read(db: fixture) -> None:
    testuser2 = User.objects.get(pk=2)
    activity_type = ActivityType.objects.get(pk=1)
    test_activity1 = Activity.objects.create(user=testuser2, activity_type=activity_type)
    test_activity2 = Activity.objects.create(user=testuser2, activity_type=activity_type)
