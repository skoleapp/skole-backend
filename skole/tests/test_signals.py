...
def test_activity_for_comment_reply(db: fixture) -> None:
    ...
    comment_reply = Comment.objects.get(pk=12) # Reply comment to the comment above, created by testuser3.
    Activity.objects.get(user=comment.user, target_user=comment_reply.user, translations__description=Activities.COMMENT_REPLY) # Throws error if it doesn't exist.

    # Test that activity is not created if user replies to his own comment.
    own_reply_comment = Comment.objects.create_comment(user=testuser2, text="test", target=comment, attachment=None)
...
def test_activity_for_course_comment(db: fixture) -> None:
    ...
    comment = Comment.objects.get(pk=9) # Comment created by testuser3, targeted to the course above.
    Activity.objects.get(user=course.user, target_user=comment.user, translations__description=Activities.COURSE_COMMENT) # Throws error if it doesn't exist.

    # Test that activity is not created if user comments his own course.
    own_course_comment = Comment.objects.get(pk=8) # Comment created by testuser2, targeted to his own course.
...
def test_activity_for_resource_comment(db: fixture) -> None:
    ...
    comment = Comment.objects.get(pk=2) # Comment created by testuser3, targeted to the resource above.
    Activity.objects.get(user=resource.user, target_user=comment.user, translations__description=Activities.RESOURCE_COMMENT) # Throws error if it doesn't exist.

    # Test that activity is not created if user comments his own resource.
    own_resource_comment = Comment.objects.get(pk=1) # Comment created by testuser2, targeted to his own resource.
