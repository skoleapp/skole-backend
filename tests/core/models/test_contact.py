...
def test_manager_create_ok(db: fixture) -> None:
    ...
    contact3 = Contact.objects.create_contact(
        contact_type=contact_type, message=message, username_or_email=email,
    )
    ...
    contact4 = Contact.objects.create_contact(
        contact_type=contact_type, message=message, username_or_email=user,
    )
...
def test_manager_create_bad_contact(db: fixture) -> None:
    ...
    with pytest.raises(TypeError):
        Contact.objects.create_contact(
            contact_type=feedback_type, message="Sending some feedback", username_or_email=1  # type: ignore[arg-type]
        )
